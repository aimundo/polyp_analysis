...
##StO2
### It seems the Fold error should just be the Standard error, not the SE divided by the mean baseline, 
#will change the formulas below accordingly
...
means1$FoldErr<-means1$StandErr
...
means2$FoldErr<-means2$StandErr
...
means3$FoldErr<-means3$StandErr
...
OxyHbmeans$FoldErr<-OxyHbmeans$StandErr
##Hb###
...
DeoxyHbmeans$FoldErr<-DeoxyHbmeans$StandErr
...
p2<-ggplot(data=means2,aes(x=time, y=Fold_change))+theme_classic()+
  geom_point(aes(color=group,group=group),size=2, position=pd,show.legend = FALSE)+
  geom_line(aes(color=group,group=group),size=1.5, position=pd,show.legend = FALSE)+
  geom_errorbar(aes(ymin=Fold_change-FoldErr,ymax=Fold_change+FoldErr,color=group),width=0.4, size=0.9, position=pd,show.legend = FALSE)+
...
p3<-ggplot(data=means3,aes(x=time, y=Fold_change))+theme_classic()+
  geom_point(aes(color=group,group=group),size=2, position=pd,show.legend = FALSE)+
  geom_line(aes(color=group,group=group),size=1.5, position=pd,show.legend = FALSE)+
  geom_errorbar(aes(ymin=Fold_change-FoldErr,ymax=Fold_change+FoldErr,color=group),width=0.4, size=0.9, position=pd,show.legend = FALSE)+
  theme(text=element_text(size=txt),aspect.ratio = 1)+
...
p4<-ggplot(data=OxyHbmeans,aes(x=time, y=Fold_change))+theme_classic()+
  geom_point(aes(color=group,group=group),size=2, position=pd,show.legend = FALSE)+
  geom_line(aes(color=group,group=group),size=1.5,position=pd,show.legend = FALSE)+
  geom_errorbar(aes(ymin=Fold_change-FoldErr,ymax=Fold_change+FoldErr,color=group),width=0.4, size=0.9,position=pd,show.legend = FALSE)+
...
p5<-ggplot(data=DeoxyHbmeans,aes(x=time, y=Fold_change))+theme_classic()+
