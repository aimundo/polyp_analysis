...
data1 %>%
  subset(GROUP=="MTD") %>%
  group_by(ID) %>%
ggplot(aes(x=DAY, y=STO2))+
  theme_bw()+
  geom_point(aes(color=factor(TUMOR)), 
             position=pd, 
             size=4, 
             alpha=0.6,
             show.legend=FALSE)+
 geom_line(aes(color=factor(TUMOR)))+
  labs(title='',
       x="Week",
       y="Oxygen \nSaturation(%)")+
...
Hmmm... How about normalizing everything by the mean value at wk 1 for each group? 

**EDIT**: Model was tried with fold change for StO2 and resulted in the exact same behavior by the smooths.
...
fold_StO2<-data1 %>%
  group_by(GROUP) %>%
  ggplot(aes(x=DAY, y=STO2/baseline_StO2))+
  theme_bw()+
  geom_point(aes(color=factor(GROUP)), 
             ...
             alpha=0.2,
             show.legend=FALSE)+
stat_summary(fun="mean",aes(colour=GROUP),geom="line",size=2,
             show.legend=FALSE)+
  labs(title='Oxygen Saturation',
       x="Week",
       y="Fold change")+
  facet_wrap(~GROUP)+thm1


fold_HbO2<-data1 %>%
  group_by(GROUP) %>%
  ggplot(aes(x=DAY, y=HBO2/baseline_HbO2))+
  theme_bw()+
 geom_point(aes(color=factor(GROUP)),
             position=pd, 
             size=4, 
             alpha=0.2,
             show.legend=FALSE)+
stat_summary(fun="median",aes(colour=GROUP),geom="line",size=2,
             show.legend=FALSE)+
  labs(title='Oxyhemoglobin',
       x="Week",
       y="Fold change")+
  facet_wrap(~GROUP)+
  thm1
...
fold_HbO<-data1 %>%
  group_by(GROUP) %>%
  ggplot(aes(x=DAY, y=HBO/baseline_HbO))+
  theme_bw()+
  geom_point(aes(color=factor(GROUP)),
             position=pd, 
             size=4, 
...
fold_tHb<-data1 %>%
  group_by(GROUP) %>%
  ggplot(aes(x=DAY, y=HB/baseline_tHb))+
  theme_bw()+
  geom_point(aes(color=factor(GROUP)),
             position=pd, 
             size=4, 
...
stat_summary(fun="median",aes(colour=GROUP),geom="line",size=2,
             show.legend=FALSE)+
  labs(title='Total Hemoglobin',
       x="Week",
       y="Fold change")+
  facet_wrap(~GROUP)+
  thm1

#plot all fold changes
(fold_StO2+fold_HbO2)/(fold_HbO+fold_tHb)+
  plot_annotation(
  tag_levels="A")&
  theme_classic()&
...
GAM with interaction of group and time. All models for fold-change values
...
pnt<-4
al<-0.2
StO2_gam <- gam(STO2/baseline_StO2 ~ GROUP+s(DAY, by = GROUP, k = 6),
            method='REML',
            data  = data1)
...
GAM_StO2_plot<-ggplot(data=data1, aes(x=DAY, y=STO2/baseline_StO2, group=GROUP)) +
  ...
  geom_line(aes(y=fit,
            ...
            data=gam_predict_StO2,
              show.legend = FALSE)+
  #facet_wrap(~Group)+
  labs(y=expression(atop(StO[2])),x="Week")+
...
```{r, HbO2-GAM}
HbO2_gam <- gam(HBO2/baseline_HbO2 ~ GROUP+s(DAY, by = GROUP, k = 6),
            method='REML',
            data  = data1)
...
#plot smooths and confidence interval for GAM
GAM_HbO2_plot<-ggplot(data=data1, aes(x=DAY, y=HBO2/baseline_HbO2, group=GROUP)) +
    geom_point(aes(color=GROUP),size=pnt,alpha=al,show.legend = FALSE)+
  geom_ribbon(aes( x=DAY,ymin=(fit - 2*se.fit), 
                   ymax=(fit + 2*se.fit),
                   fill=GROUP
  ...
  geom_line(aes(y=fit,
            ...
            data=gam_predict_HbO2,
              show.legend = FALSE)+
  #facet_wrap(~Group)+
  labs(y=expression(atop(HbO[2])),x="Week")+
    scale_x_continuous(breaks=c(0,2,4,6))+
      theme_classic()+
 thm1
...
```{r, tHb-GAM}
tHb_gam <- gam(HB/baseline_tHb ~ GROUP+s(DAY, by = GROUP, k = 6),
            method='REML',
            data  = data1)
...
#plot smooths and confidence interval for GAM
GAM_tHb_plot<-ggplot(data=data1, aes(x=DAY, y=HB/baseline_tHb, group=GROUP)) +
    geom_point(aes(color=GROUP),size=pnt,alpha=al,show.legend = FALSE)+
  geom_ribbon(aes( x=DAY,ymin=(fit - 2*se.fit), 
                   ymax=(fit + 2*se.fit),
                   fill=GROUP
...
HbO_gam <- gam(HBO/baseline_HbO ~ GROUP+s(DAY, by = GROUP, k = 6),
            method='REML',
            data  = data1)
...
#plot smooths and confidence interval for GAM
GAM_HbO_plot<-ggplot(data=data1, aes(x=DAY, y=HBO/baseline_HbO, group=GROUP)) +
    geom_point(aes(color=GROUP),size=pnt,alpha=al,show.legend = FALSE)+
  geom_ribbon(aes( x=DAY,ymin=(fit - 2*se.fit), 
                   ymax=(fit + 2*se.fit),
                   fill=GROUP
  ...
  geom_line(aes(y=fit,
            ...
            data=gam_predict_HbO,
              show.legend = FALSE)+
  #facet_wrap(~Group)+
  labs(y=expression(atop(HbO)),x="Week")+
    scale_x_continuous(breaks=c(0,2,4,6))+
      theme_classic()+
   thm1
...
smooth_diff <- function(model, newdata, g1, g2, alpha = 0.05,
    ...
    X <- xp[row1, ] - xp[row2, ]
    ## zero out cols of X related to splines for other groups
    X[, ! (col1 | col2)] <- 0
...
comp1<-smooth_diff(StO2_gam,pdat,'CG','MET')
...
(HbO_CG_MG+HbO_CG_MTD+HbO_MG_MTD)&plot_annotation(title="Pairwise Smooth Comparisons")&
